...
print_usage() {
  ...
  echo "                                  - shard_capacity: Check total shard usage vs. the cluster.max_shards_per_node limit."
...
check_shard_capacity() {
  ...
  local warn="${WARN_THRESHOLD:-80}"   # % used
  local crit="${CRIT_THRESHOLD:-95}"   # % used

  # Pull cluster stats (has both node counts and shard totals)
  local stats
  stats=$(execute_curl "$OPENSEARCH_URL/_cluster/stats")

  # Total shards (primaries + replicas) currently open/active
  local total_shards
  total_shards=$(echo "$stats" | jq -r '.indices.shards.total // 0')

  # Data node count
  local data_nodes
  data_nodes=$(echo "$stats" | jq -r '.nodes.count.data // 0')

  if [[ -z "$data_nodes" || "$data_nodes" -le 0 ]]; then
    echo "CRITICAL: No data nodes reported by cluster - OpenSearch cluster likely unhealthy or unreachable."
    exit 2
  fi

  # Get max_shards_per_node from settings (persistent -> transient -> defaults)
  local settings
  settings=$(execute_curl "$OPENSEARCH_URL/_cluster/settings?include_defaults=true")

  # Try persistent, then transient, then defaults
  local mspn
  mspn=$(echo "$settings" | jq -r '
      .persistent.cluster.max_shards_per_node // .transient.cluster.max_shards_per_node // .defaults.cluster.max_shards_per_node // empty
    ')
  # Fallback to 1000 if empty
  if [[ -z "$mspn" || "$mspn" == "null" ]]; then
    mspn=1000
  fi

  # Compute cluster-wide ceiling
  ...
  # Used percentage
  local used_pct
  used_pct=$(echo "scale=2; ($total_shards*100)/$max_total" | bc)
  # Headroom (absolute shards)
  local headroom=$(( max_total - total_shards ))

  # Perfdata
  local perf="total_shards=$total_shards;;;$max_total; max_shards=$max_total;;;0; used_pct=${used_pct}%;$warn;$crit;0;100 headroom=$headroom;;;0;"

  # Evaluate thresholds
  # Compare as decimals via bc
  if (( $(echo "$used_pct >= $crit" | bc -l) )); then
    echo "CRITICAL: Shard capacity used ${used_pct}% ($total_shards/$max_total). per_node=$mspn data_nodes=$data_nodes headroom=$headroom | $perf"
    exit 2
  elif (( $(echo "$used_pct >= $warn" | bc -l) )); then
    echo "WARNING: Shard capacity used ${used_pct}% ($total_shards/$max_total). per_node=$mspn data_nodes=$data_nodes headroom=$headroom | $perf"
    exit 1
  else
    echo "OK: Shard capacity used ${used_pct}% ($total_shards/$max_total). per_node=$mspn data_nodes=$data_nodes headroom=$headroom | $perf"
    exit 0
  fi
}
