class nagios::check::cpu_persistent (
  ...
  Optional[String]          $warn                     = '90',
  Optional[String]          $crit                     = '95',
  ...
  # Deploy the check script (remove it when ensure is 'absent')
  file { '/usr/lib64/nagios/plugins/check_cpu_persistent':
    ...
    mode   => '0755',
    source => $ensure ? {
      'present' => "puppet:///modules/${module_name}/scripts/check_cpu_persistent",
      'absent'  => undef,
    },
  }

  # Configure NRPE file for the new plugin (remove it when ensure is 'absent')
  ...
  # Define the Nagios service (remove it when ensure is 'absent')
